...
for (n in Ns) {
  for (p in Ps) {
    file_path <- paste0(data_dir, "FCIR_data_Dense_N", n, "_P", p, ".Rdata")
...
p1 <- ggplot(all_results, aes(x = N, y = Sparsity, fill = P)) +
  geom_boxplot() +
  labs(title = "Species Sparsity Distribution (Dense Data)",
       x = "Sample Size (N)",
...
plot1_path <- paste0(out_dir, "Sparsity_Species_Boxplot_Dense.png")
...
p2 <- ggplot(overall_df, aes(x = as.numeric(as.character(N)), y = Overall_Sparsity, color = P, group = P)) +
  ...
  geom_point(size = 3) +
  labs(title = "Overall Sparsity Trend (Dense Data)",
       x = "Sample Size (N)",
       y = "Overall Sparsity",
...
plot2_path <- paste0(out_dir, "Sparsity_Overall_Trend_Dense.png")
...
p3 <- ggplot(all_site_results, aes(x = N, y = Sparsity, fill = P)) +
  geom_boxplot() +
  labs(title = "Site Sparsity Distribution (Dense Data)",
       x = "Sample Size (N)",
...
plot3_path <- paste0(out_dir, "Sparsity_Site_Boxplot_Dense.png")
